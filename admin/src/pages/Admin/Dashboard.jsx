    ...
    <div className='w-full p-4 sm:p-8 text-left text-slate-800'>
      ...
      <div className='flex flex-col items-start gap-1.5 mb-8'>
        ...
        <h1 className='text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight mt-1'>
          System <span className='font-serif-accent italic font-normal text-teal-700'>Dashboard</span>
        </h1>
        <p className='text-sm sm:text-base text-slate-600 font-medium'>
          Real-time metrics on doctors, total appointments, and registered patients.
        </p>
      </div>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8'>
        ...
        <div className='bg-white p-6 rounded-3xl border border-emerald-100/90 shadow-sm hover:shadow-xl hover:shadow-teal-900/5 hover:-translate-y-1 transition-all duration-300 flex items-center gap-5 cursor-pointer'>
          ...
          <div className='flex flex-col'>
            <span className='text-3xl sm:text-4xl font-extrabold text-slate-900'>{dashData.doctors}</span>
            <span className='text-xs sm:text-sm font-bold uppercase tracking-wider text-slate-400 mt-0.5'>Total Doctors</span>
          </div>
        </div>
        ...
        <div className='bg-white p-6 rounded-3xl border border-emerald-100/90 shadow-sm hover:shadow-xl hover:shadow-teal-900/5 hover:-translate-y-1 transition-all duration-300 flex items-center gap-5 cursor-pointer'>
          ...
          <div className='flex flex-col'>
            ...
            <span className='text-xs sm:text-sm font-bold uppercase tracking-wider text-slate-400 mt-0.5'>Registered Patients</span>
          </div>
        </div>
      ...
      <div className='w-full bg-white rounded-3xl border border-emerald-100/90 shadow-sm overflow-hidden'>
        <div className='flex items-center gap-3 px-6 py-5 bg-gradient-to-r from-emerald-50 to-teal-50/40 border-b border-emerald-100'>
          ...
          <h2 className='font-extrabold text-slate-900 text-lg sm:text-xl tracking-tight'>Latest Bookings</h2>
        </div>
          ...
          {dashData.latestAppointments.map((item, index) => (
            <div className='flex items-center justify-between px-6 py-4.5 hover:bg-emerald-50/30 transition-colors' key={index}>
              <div className='flex items-center gap-4'>
                <img className='rounded-2xl w-12 h-12 object-cover border border-emerald-100 bg-emerald-50' src={item.docData?.image} alt="" />
                <div className='flex flex-col text-sm sm:text-base'>
                  <p className='text-slate-900 font-extrabold'>{item.docData?.name}</p>
              ...
              <div>
                {item.cancelled ? (
                  ...
                  <button 
                    ...
                    className='px-3.5 py-1.5 rounded-xl bg-slate-100 hover:bg-red-50 text-slate-600 hover:text-red-600 transition-colors text-xs sm:text-sm font-bold border border-slate-200'
                  >
                    Cancel ✕
                  </button>
                )}
